...
if parent_folder.startswith('~'): p = Path(parent_folder).expanduser()
else:                             p = Path(parent_folder)
...
for d in s_dirs:
    ...
    if not iscluster: 
        ...
        exit()
        # There was a very complex FindEdge function here by Akash using sobel
        # but too complicated. XD
        print("number of iterations:",end="")
        # Remember if end = "", buffer won't flush automatically!!
        # so no printing before tqdm
        results = []
        for img in tqdm(images,  desc = 'Chan-Vese Processing'):
            # Akash's Chan-Vese didn't have tol value, but I dont want to wait 100 loops everytime.
...
speed_df.to_csv(p / 'average_speed.csv')
order_df.to_csv(p / 'order_parameter.csv')
